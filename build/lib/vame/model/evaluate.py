...
def plot_reconstruction(filepath, test_loader, seq_len_half, model, model_name, 
    ...
    if FUTURE_DECODER:
        for t in range(len(data_orig)-1):
            fig, (ax1, ax2) = plt.subplots(1, 2)
            fig.suptitle('Reconstruction and future prediction of input sequence')
            ax1.plot(data_orig[t,...], color='k', label='Sequence Data')
            ax1.plot(data_tilde[t,...], color='r', linestyle='dashed', label='Sequence Reconstruction') 
            ax2.plot(fut_orig[t,...], color='k')
            ax2.plot(fut[t,...], color='r', linestyle='dashed')
            fig.savefig(filepath+'evaluate/'+str(t)+'Future_Reconstruction.png') 
...
# two of the upper plots included
def eval_temporal(cfg, use_gpu, model_name):
    ...
    TEST_BATCH_SIZE = 64 # we can change this and get more evaluation. they are just of time window size
