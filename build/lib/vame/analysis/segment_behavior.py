...
# return Z score but are they based on vec? 
...
# n_int is the number of iteration from which centrois are calculated, we can higher the number whitout worry.
#random stat is the num of intialization since no absolut minima is promissed , we can higher this 
...
# return the parameter of multy dim gaussian (EM inside)
def gmm_clustering(context,n_components):
    GMM = mixture.GaussianMixture
    gmm = GMM(n_components=n_components,covariance_type='full').fit(context) # no reduction (ncomp = ncomp)
    return gmm.predict(context)

# the function we call that holds all.....
...
def temporal_quant(cfg, model_name, files, use_gpu):
    ...
    for file in files:
        ...
        with torch.no_grad(): 
            for i in range(num_frames):
                ...
                data = np.reshape(data, (1,temp_win,NUM_FEATURES))## I ADD  -1!!!!
    ...
    # save for clusterin and more independencies 
    np.save(cfg['project_path']+'/'+'model/z_array' , z_array)
...
def cluster_latent_space(cfg, files, z_data, z_logger, cluster_method, n_cluster, model_name):
    ...
    for cluster in n_cluster:
        if cluster_method == 'kmeans':
            ...
            '''
            # add time vec for the classifier to learn also by time 
            counter_list = []
            for file in range(0,210):
                [counter_list.append(i/2355) for i in range(0,2355)]
            counter_np = np.array(counter_list, dtype=np.int64)
            z_with_time = np.c_[z_data, counter_np]
            ...
            data_labels = kmeans_clustering(z_data, n_clusters=cluster) # chane from z_data -> z_with_time
            data_labels = np.int64(scipy.signal.medfilt(data_labels, cfg['median_filter']))
            np.save(cfg['project_path']+'/'+'model/data_labels'+str(cluster) , data_labels)
            
        if cluster_method == 'GMM':
            print('Behavior segmentation via GMM.')
            data_labels = gmm_clustering(z_data, n_components=cluster) # chane from z_data -> z_with_time
            data_labels = np.int64(scipy.signal.medfilt(data_labels, cfg['median_filter']))
        ...
        for idx, file in enumerate(files):
            ...
            labels = data_labels[z_logger[idx]:z_logger[idx+1]] # z_logger is just a vector for start-end per animal, to cut it to files, idx 
...
#config = "I:/VAME_projects/minmax_LowerBach_Rightcam-Mar16-2021/config.yaml"
#behavior_segmentation(config, model_name='VAME', cluster_method='kmeans', n_cluster=[15, 20, 30])